...
def obj_loss(pred, true):
    ...
    obj_true = true[..., 4]
    return -tf.reduce_mean((obj_true * tf.math.log(obj_pred) + (1 - obj_true) * tf.math.log(tf.clip_by_value(1 - obj_pred, 1e-9, 1e+9))) * obj_true)
...
def nobj_loss(pred, true, ignore_mask, tune_param):
    ...
    obj_pred = tf.clip_by_value(pred[...,4], 1e-9, 1e+9)
    obj_true = true[..., 4]
    return -tf.reduce_mean((obj_true * tf.math.log(obj_pred) + ((1 - obj_true) * tf.math.log(tf.clip_by_value(1 - obj_pred, 1e-9, 1e+9)))) * ((1-obj_true) * ignore_mask)) * noobj_tune
...
def cls_loss(pred, true):
    ...
    cls_true = true[..., 5:]
    obj_mask = true[..., 4]
    return -tf.reduce_mean(tf.reduce_sum(cls_true * tf.math.log(cls_pred) + (1 - cls_true) * tf.math.log(tf.clip_by_value(1 - cls_pred, 1e-9, 1e+9)), axis=-1) * obj_mask)
#%%